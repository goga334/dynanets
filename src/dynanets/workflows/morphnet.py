...
class MorphNetWorkflow(MethodWorkflow):
    sparse_epochs: int = 6
    finetune_epochs: int = 4
    sparsity_strength: float = 4e-4
    target_flop_reduction: float = 0.30
    max_prune_fraction: float = 0.35
    min_channels_per_block: int = 8
    metric_name: str = "accuracy"

    def __post_init__(self) -> None:
        if self.sparse_epochs <= 0:
            raise ValueError("MorphNetWorkflow requires sparse_epochs > 0")
        if self.finetune_epochs < 0:
            raise ValueError("MorphNetWorkflow finetune_epochs must be non-negative")
        if self.sparsity_strength < 0.0:
            raise ValueError("MorphNetWorkflow sparsity_strength must be non-negative")
        if not 0.0 < self.target_flop_reduction < 1.0:
            raise ValueError("MorphNetWorkflow target_flop_reduction must be in (0.0, 1.0)")
        if not 0.0 < self.max_prune_fraction < 1.0:
            raise ValueError("MorphNetWorkflow max_prune_fraction must be in (0.0, 1.0)")
        if self.min_channels_per_block <= 0:
            raise ValueError("MorphNetWorkflow min_channels_per_block must be positive")
    ...
    def execute(
        ...
        *,
        ...
        if adaptation is not None:
            raise ValueError("MorphNetWorkflow manages pruning internally and does not accept adaptation")
        if not isinstance(model, TorchCNNClassifier):
            raise ValueError("MorphNetWorkflow currently supports TorchCNNClassifier only")
        if not model.spec.use_batch_norm:
            raise ValueError("MorphNetWorkflow requires a batch-normalized CNN")

        configured_total = self.sparse_epochs + self.finetune_epochs
        if epochs != configured_total:
            raise ValueError(f"MorphNetWorkflow requires trainer.epochs={configured_total}, got {epochs}")
        ...
        self._extend_stage(
            ...
            stage_name="morphnet_resource_train",
            epoch_start=cursor + 1,
            metadata={"phase": "resource_train", "sparsity_strength": self.sparsity_strength},
        )
        ...
        before_flops = int(before_meta.get("forward_flop_proxy", 0))
        prune_fraction = min(self.max_prune_fraction, max(0.05, self.target_flop_reduction))
        ...
        after_flops = int(after_meta.get("forward_flop_proxy", 0))
            ...
            summary.adaptation_history.append(
                AppliedAdaptationEvent(
                    ...
                    metadata={
                        ...
                        "target_flop_reduction": self.target_flop_reduction,
                        "estimated_flop_reduction": _safe_ratio(before_flops - after_flops, before_flops),
                    },
        ...
        summary.workflow_metadata = {
            "workflow_name": "morphnet",
            "configured_total_epochs": epochs,
            "executed_total_epochs": cursor,
            "stage_count": len(summary.stage_history),
            "target_flop_reduction": self.target_flop_reduction,
            "before_conv_channels": before_meta.get("conv_channels"),
            "after_conv_channels": after_meta.get("conv_channels"),
            "before_flops": before_flops,
            "after_flops": after_flops,
        }
        return summary
